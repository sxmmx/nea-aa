...
def login(User_name, User_password):
      logintodb(User_name, User_password)  # Call the logintodb function with the provided username and password
...
def openNewWindow6():
  ...
  summary_text = """Protein synthesis is the creation of protein using DNA. This is done by RNA Polymerase (an enzyme) breaking hydrogen bonds between the complementary bases( A, T, C, and G), causing the DNA to break into the sense strand and antisense strand (this references the direction the strand on nucleotides are facing). The antisense strand will be used as a template strand and free nucleotides (basic structural unit of nucleic acids like DNA) will line up against complementary base pairs and join together by phosphodiester bonds(between nucleotides-), catalysed by RNA polymerase. This forms messenger RNA, known as mRNA. This moves out of the nuclear pore into the cytoplasm, thus ending transcription.
When the mRNA is attached to a ribosome in the cytoplasm, translation starts. Transfer RNA (tRNA) is a molecule that binds amino acids(the molecule that makes protein when combined) together based on anticodons (three complementary bases to mRNA bases ). Anticodons on tRNA bind to codons on mRNA and are held together by hydrogen bonds. Amino acids in the cytoplasm attach to tRNA and ribosomes attach to amino acids on two tRNA molecules using peptide bonds before tRNA detaches from amino acids. This process repeats until a stop codon is reached. The amino acids can fold into a secondary or tertiary structure, with the primary structure is the amino acids not changing shape after being synthesised. Secondary and tertiary structures are proteins in complex 3D form. This is so the proteins can carry out specific functions, like releasing hormones.
"""
